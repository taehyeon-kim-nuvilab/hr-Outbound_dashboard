  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-6">채용 퍼널</h2>
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3 mb-6">
              {funnel.map((item, idx) => (
                <div
                  key={item.stage}
                  className={`bg-gradient-to-b ${funnelColors[idx]} rounded-xl p-4 flex flex-col items-center justify-center text-center shadow-sm`}
                >
                  <p className={`text-xs font-medium mb-1 ${funnelTextColors[idx]} opacity-80`}>
                    {item.label}
                  </p>
                  <p className={`text-3xl font-bold ${funnelTextColors[idx]}`}>
                    {item.count}
                  </p>
                  <p className={`text-xs mt-1 ${funnelTextColors[idx]} opacity-70`}>
                ...
                return (
                  <div key={item.stage} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 bg-gray-100 rounded-full h-5 overflow-hidden">
                      <div
                        className={`${barColors[idx]} h-full rounded-full transition-all duration-500 flex items-center justify-end pr-2`}
                        style={{ width: `${Math.max(widthPct, widthPct > 0 ? 3 : 0)}%` }}
                      >
                        {item.count > 0 && (
